...
pub struct Overview {
    ...
    /// Opacity for windows that do not match the overview filter (0.0 = hidden, e.g. 0.3 = dim).
    pub filter_dim_opacity: f64,
}
...
pub struct OverviewPart {
    ...
    #[knuffel(child, unwrap(argument))]
    pub filter_dim_opacity: Option<FloatOrInt<0, 1>>,
}

impl MergeWith<OverviewPart> for Overview {
    fn merge_with(&mut self, part: &OverviewPart) {
        merge!((self, part), zoom, workspace_shadow, filter_dim_opacity);
